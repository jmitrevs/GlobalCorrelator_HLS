...
int main() {
    ...
    for (int test = 1; test <= NTEST; ++test) {
        ...
        for (int i = 0; i < NOUT_SORT; i++) {
            data_out[2*i+0] = output_apx[i](31,0);
            data_out[2*i+1] = output_apx[i](63,32);
        }
        //PFOutputObj pf_sort_internal[NOUT_SORT];
        //sort_output_onlycands(pf_comb_internal,pf_sort_internal);
        //mp7wrapped_pack_out_comb<NOUT_SORT>(pf_sort_internal, data_out);
